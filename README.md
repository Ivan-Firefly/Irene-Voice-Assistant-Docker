...
Для удобства есть скрипт `download.sh`, который все скачает и разместит по нужным папкам. Сделать его исполняемым ```chmod +x download.sh``` и запускать ```./download.sh```
