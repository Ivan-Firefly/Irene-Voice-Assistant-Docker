...
Добавлен `docker-compose.yml` для удобного запуска контейнера. После старта в текущей директории создается папка `docker_options` куда выносятся основные настройки Ирины и запущеных плагинов.
